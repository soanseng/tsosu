...
- **🌐 繁中介面全數補齊**——清除所有硬編英文：底部分頁、設定全部區塊與狀態訊息（備份／匯出／還原、同步與匯入回饋）、速記的優先級／精力／重複選項、任務與習慣詳情、選一個、搜尋、行事曆空狀態。
- **📁 資料夾選擇更順**——系統選擇器直接開在「文件」而非死路的根目錄檢視；已連結的 vault 卡片新增「更換資料夾」按鈕，不必再中斷後重選。

## 1.2.1 新功能

- **🛠 實機淬鍊**：1.2.0 實機測試發現的七項修復——速記的 `@專案`／`due:` 語法不再殘留在標題（並顯示偵測晶片）、每週回顧精靈可從檢視選單進入、習慣連續天數勾選即時更新、JSON 備份不再寫出空檔案、ICS 訂閱真的會抓取並疊加日曆（含開關）、應用鎖冷啟動確實上鎖。
- **🔁 往返與時間**：Vault 筆記保留 `estimate` 欄位；每日摘要重新錨定 08:00/20:00 不再漂移；番茄鐘進行中按鈕不再顯示「開始」；收件匣批次刪除先詢問。

## 1.2.0 新功能

- **🔗 隨手捕獲**：從任何 App 分享文字直達速記；快捷設定磁貼一鍵開始；速記內建語音輸入。
- **✍️ 速記新語法**：`@專案` 自動歸檔；`due:今天`／`due:下週`／`due:8/31` 直接設到期日。
- **🔍 搜尋運算子**：頂欄搜尋支援 `status:done`、`due:<=7d`、`due:overdue`、`#tag`。
- **🍅 Focus 番茄鐘**：15/3、25/5、50/10 預設，專注分鐘自動記錄到任務。
- **🔔 更體貼的提醒**：通知可稍後 10 分、開始時間獨立提醒、每天早晚溫和摘要。
- **📦 批次與備份**：收件匣多選完成／改天／刪除；設定可完整 JSON 備份與還原。
- **📅 日曆訂閱**：訂閱外部 .ics 行事曆（色點顯示）；**TickTick CSV 匯入**；生物辨識 App 鎖。
- **🛠 穩定性**：通知與小工具完成循環任務會正確展開；提醒在 App 更新與時區變更後自動重排；.ics 合乎 RFC 5545；連續紀錄跨時區穩定（資料庫 v12，自動遷移）。
...
- 有 🔁 重複規則的任務就是習慣——會出現在習慣頁的「循環任務」區塊
...
- 支援 **CalDAV**（Fastmail、Nextcloud）和 **Google Calendar**——也能把 `.ics` 發布到任何 **WebDAV** 伺服器
