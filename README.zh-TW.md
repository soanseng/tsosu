...
- **🗂 更安靜的首頁**——收件匣是起點；app 就是四個分頁（收件匣／今天／習慣／即將到來）加收進頂欄選單的日曆與分類。番茄鐘、看板、每週回顧、儲存檢視已移除——刻意的。
...
- **🗂 分類**——在快速新增列、`@分類` 語法或詳情頁的晶片給任務一個分類（論文、自我成長…）；頂欄 ⌄ 選單多了**分類**檢視，把未完成任務依分類分組。
- **✏️ 時段可編輯**——🌅／☀️／🌙 時段建立後也能改，不再只能新增時設定。
...
- **收件匣**——所有還沒排期的任務：無分類的在上，各分類折疊在下面。清空它，不用怕它。
...
- **分類**——同樣在 ⌄ 選單：所有未完成任務依分類分組，最後一組是「未分類」。分類就是 vault 裡的 project，跟其他資料一樣能與 Obsidian 往返。
- **先看再改**——任務列會顯示描述的第一行，多數情況不用點開。點任務進入唯讀檢視（狀態、日期、分類、描述，以及描述裡的所有連結）；真的要改再按「編輯」。
...
| `🆔 id` | 任務穩定 id（也相容舊版 `<!-- id:... -->`） |
| `⛔ id1,id2` | 等待其他任務 |
| `🔺 ⏫ 🔼 🔽 ⏬` | 優先級——註：Tsosu 把 `⏫` 視為最高級（緊急），比 Obsidian 對同一 emoji 的語意高一級 |
